...
import { DateTime, Duration } from "luxon"; // also: Interval, Settings, Zone
import { PerformanceTimer } from "./perf-timer"; // exported as is
...
import {
  ...
  MonthLabel,
  TimerInterval,
  DefaultDateTimeObjectLabels,
  DateTimeObjectLabelTranslationKeys,
  DateTimeValue,
  TimeObject,
  DateTimeObjectLabels,
  PluralLabel,
} from "../constants";
 ...
 */
const coerceMinutesFormatArgument = function (candidate: string) {
  const validFormats = DateTimeFormats.Time.MinsSecs;
  let result = validFormats.asTime;
  // only one pathway will reset the result, all others return 'asTime'
  if (candidate) {
    if (
      candidate === validFormats.asTime ||
...
const _constants = {
  ...
  dateTimeObjectFormatLabels: {
    default: DefaultDateTimeObjectLabels,
    translationKeys: DateTimeObjectLabelTranslationKeys,
  },
  ENUMS: {
 ...
 */
const _timeCategoryInDays = function (category: string) {
 ...
 */
const _keyNameForDayOfWeek = function (dayOfWeek: number) {
  const dayKeys = [
    "sunday",
    "monday",
    "tuesday",
  ...
  throw new Error(
    `TimeUtils.keyNameForDayOfWeek expected a value between 1 and 7, got ${dayOfWeek}`,
  );
};
 ...
 */
const _isoFormattedDate = function (dateItem: Date | string | number) {
  return _formattedDate(dateItem, DateTimeFormats.ISO);
};
 ...
 */
const _sqlFormattedDate = function (dateItem: Date | string | number) {
  return _formattedDate(dateItem, DateTimeFormats.SQL);
};
 ...
 */
const _reportFormattedDate = function (
  dateOrString: Date | string,
  shouldAppendTime: boolean,
): string {
  let result = "";
  let epochMillis = 0;
  try {
  ...
  if (epochMillis > 0) {
    const outputFormat =
      shouldAppendTime === true
        ? DateTimeFormats.DateTime.medium24Hr
 ...
 */
const _reportFormattedDateArray = function (
  dateArray: Array<Date | string>,
  shouldAppendTime: boolean,
) {
  if (IsThis(dateArray).anArray === false) {
    throw new Error(
      "TimeUtils.reportFormattedDateArray() was expecting an Array of values to format. Got: " +
        typeof dateArray,
    );
  }
 ...
 */
function _formattedDate(dateItem: DateTimeValue, format: string) {
  const ms = DateTimeParser.parseValueOrNow(dateItem);
  const dt = DateTime.fromMillis(ms).setZone(DefaultTimeZoneName);
 ...
 */
const _timeObjectFromSeconds = function (seconds: number) {
  const result = Duration.fromObject({ hours: 0, minutes: 0, seconds: seconds })
    .normalize()
    .toObject();
 ...
 */
const _timeFormatFromSeconds = function (seconds: number) {
  const result = Duration.fromMillis(seconds * 1000).toFormat("hh:mm:ss");
  return result;
};
 ...
 */
const _minutesFromSeconds = function (seconds: number, toFormat: string) {
  if (!IsThis(seconds).aUseableNumber) {
    throw new Error(
      "TimeUtils.minutesFromSeconds was expecting a valid number of Seconds",
    );
  }
 ...
 * @return {string} the input value in mins & secs
 */
const _minutesFromMilliseconds = function (
  milliseconds: number,
  toFormat: string,
) {
  if (!IsThis(milliseconds).aUseableNumber) {
    throw new Error(
      "TimeUtils.minutesFromMilliseconds was expecting a valid number of milliseconds",
    );
  }
 ...
 */
const _minutesToMilliseconds = function (mins: number) {
  if (IsThis(mins).aUseableNumber === false) return 0;
 ...
 */
const _monthOfDate = function (dateOrString: Date | string) {
 ...
 */
const _getNewDateHelper = function (dateItem: DateTimeValue) {
  const result = new DateHelper(dateItem);
  return result;
};
...
/**
  ...
  */
const _getTwoHighestNonZeroOrdinals = function (
  dateItem1: DateTimeValue,
  dateItem2: DateTimeValue,
  ordinals?: DateTimeObjectLabels,
) {
  const d = new DateHelper(dateItem1).difference(dateItem2).toObject;

  // if values not supplied, use the basic [en] key names defined in TypeDefs
  if (!ordinals) ordinals = DefaultDateTimeObjectLabels;

  const nouns = (count: number, ordinal: PluralLabel) => {
...
/**
  ...
  * @example const d = TimeUtils.getTwoHighestNonZeroOrdinals(
       ...
       returns '4 months'
  */
const _autoFormatDuration = function (
  dateItem1: DateTimeValue,
  dateItem2: DateTimeValue,
  ordinals?: DateTimeObjectLabels,
) {
  ...
  const nouns = (count: number, value: PluralLabel) => {
  ...
  if (d.year > 0) {
    ...
    if (d.month > 0) result.push(nouns(d.month, ordinals.month));
  } else if (d.month > 0) {
    result.push(nouns(d.month, ordinals.month));
    if (d.day) result.push(nouns(d.day, ordinals.day));
  } else if (d.day > 0) {
    result.push(nouns(d.day, ordinals.day));
    if (d.hour) result.push(nouns(d.hour, ordinals.hour));
  } else if (d.hour > 0) {
    result.push(nouns(d.hour, ordinals.hour));
    if (d.minute) result.push(nouns(d.minute, ordinals.minute));
  } else if (d.minute > 0) {
    result.push(nouns(d.minute, ordinals.minute));
    if (d.second > 0) result.push(nouns(d.second, ordinals.second));
  } else if (d.second > 0) result.push(nouns(d.second, ordinals.second));
 ...
 *
 */
export const timeConstants = _constants;
export const timeCategoryInDays = _timeCategoryInDays;
export const keyNameForDayOfWeek = _keyNameForDayOfWeek;
export const isoFormattedDate = _isoFormattedDate;
export const sqlFormattedDate = _sqlFormattedDate;
export const reportFormattedDate = _reportFormattedDate;
export const reportFormattedDateArray = _reportFormattedDateArray;
export const formattedDate = _formattedDate;
export const timeObjectFromSeconds = _timeObjectFromSeconds;
export const timeFormatFromSeconds = _timeFormatFromSeconds;
export const minutesFromSeconds = _minutesFromSeconds;
export const minutesFromMilliseconds = _minutesFromMilliseconds;
export const minutesToMilliseconds = _minutesToMilliseconds;
export const monthOfDate = _monthOfDate;
export { PerformanceTimer };
export const date = _getNewDateHelper;
export const months = new MonthLabelHelper();
export const getTwoHighestNonZeroOrdinals = _getTwoHighestNonZeroOrdinals;
export const autoFormatDuration = _autoFormatDuration;
